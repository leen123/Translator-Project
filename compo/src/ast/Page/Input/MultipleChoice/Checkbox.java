...
public class Checkbox{
        ...
        if(SymbolTable.checkFound(Key,"","global").size()<=0){
            ...
            if(SymbolTable.checkFound(row.table,Key,"","").size()<=0){
                ...
                for (InputText inputText:inputTexts) {
                    inputText.debug(tempPath,"checkbox");
                }
            }else {
                ErrorHandling.error.add(Colors.TEXT_B+Colors.TEXT_RED+"[a name '"+Key+"' is found in this scope'"+scope+"']:"+Colors.TEXT_RESET+Colors.TEXT_RED+" can't define the checkbox by that name"
                        + Colors.TEXT_YELLOW+ ", path: '"+SymbolTable.formatPath(tempPath)+"'"+Colors.TEXT_RESET);
                SymbolTable.stop=true;
            }
        }else {
            ErrorHandling.error.add(Colors.TEXT_B+Colors.TEXT_RED+"[a name '"+Key+"' is found in the scope'"+scope+"']:"+Colors.TEXT_RESET+Colors.TEXT_RED+" can't define the checkbox by that name"
                    + Colors.TEXT_YELLOW+ ", path: '"+SymbolTable.formatPath(tempPath)+"'"+Colors.TEXT_RESET);
            SymbolTable.stop=true;
        }
    ...
    public String generating(String space, boolean required, Location location){
        ...
        for (InputText inputText:inputTexts) {
            ...
            codeGenerating+="type=\"checkbox\" id=\""+inputText.generating()+"\" name=\""+getKey()+"\" value=\""+inputText.generating()+"\">\n";
            codeGenerating+=space+"\t\t"+"<label for=\""+inputText.generating()+
                    "\">"+inputText.generating()+"</label>\n";
        }
